...
def fetch_food_log(session: requests.Session, date: str, meal_type_id: int) -> FoodLog:
    """
    Fetch food log for a specific date and meal type.
    Args:
        session (requests.Session): Authenticated session.
        date (str): Date in the format 'YYYY-MM-DD'.
        meal_type_id (int): Meal type ID (3: 朝食, 4: 昼食, 5: 夕食, 6: 間食).
    Returns:
    ...
    advice_url = f"{ASKEN_URL}/wsp/advice/{date}/{meal_type_id}"
    response = session.get(url=advice_url, headers=headers)
    response.raise_for_status()  # Check if the request was successful
